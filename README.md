...
Every day at **~08:00 Singapore time** the pipeline collects from ~20 free sources (regulators, arXiv, trade media, Google News topical sweeps), dedupes and filters, has Claude write a ~1,500–2,000-word decision-oriented brief with traceable citations, and publishes Markdown + HTML.
...
- The cron fires at **23:47 UTC (07:47 SGT)** — off the congested midnight-UTC slot — and GitHub's queue delay lands the brief around **8:00–8:30 AM SGT**. Reports are dated in Singapore time. If synthesis fails (e.g. no API credits), the run shows red but collected items are still committed.
